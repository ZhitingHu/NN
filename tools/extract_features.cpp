...
int feature_extraction_pipeline(int argc, char** argv) {
  ...
  Caffe::initialize_phases(FLAGS_num_table_threads);
  for (int tidx = 0; tidx <FLAGS_num_table_threads; ++tidx) {
  ...
  LOG(INFO) << "Starting feature extraction with " << FLAGS_num_table_threads
